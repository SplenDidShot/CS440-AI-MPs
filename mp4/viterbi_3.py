...
def collect_init_trans_emission_stats(train):
    ...
    for sentence in train:
        seen_words.add(word for word in sentence)
    ...
    return init_tag_counts, tag_pair_counts, tag_word_counts, hapax_words, seen_words
...
def compute_init_transition_emission_probabilities(init_tag_probs, tag_pair_probs, tag_word_probs, hapax_probs,
    ...
    hapax_smoothing = smoothing_param
    denominator = total_hapax_count + hapax_smoothing * total_hapax_tags
    for tag, tag_count in hapax_probs.items():
        hapax_probs[tag] = math.log((tag_count + hapax_smoothing)/denominator)
    hapax_unseen = math.log(hapax_smoothing/denominator)
...
def viterbi_3(train, test):
    ...
    init_tag_counts, tag_pair_counts, tag_word_counts, hapax_words, seen_words = collect_init_trans_emission_stats(train)
    ...
    for sentence in test:
        ...
        best_path_mod = []
        noun_set = {'tor', 'ism', 'ury',
                    'urg', 'nia', 'ips', 'nts', 'oro', 'ary', 'tes', 'ate', 'ana',
                    'nce', 'ess', 'gle', 'nes',
                    'one', 'ria', 'ion', 'ist', 'vel',
                    'mer', 'ity', 'ter', 'ers', 'age'}
        for word, tag in best_path:
            if word in seen_words:
                best_path_mod.append((word, tag))
                continue
            if len(word) > 5 and any(ending in word[-3:] for ending in noun_set):
                best_path_mod.append((word,'NOUN'))
            else:
                best_path_mod.append((word,tag))

        tag_result.append(best_path_mod[::-1])
